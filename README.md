...
**抖音**：拉取 HLS origin 流，弹幕通过官方 WebSocket 协议实时接收；

**虎牙**：通过解析直播页 JS 数据获取签名 HLS 流，暂未接入弹幕；

**快手**：优先 HLS，不可用时自动回退 FLV。弹幕通过 WebSocket 接收；

**TikTok（海外）**：需要代理访问，支持按用户名 `@xxx` 或数字房间 ID 添加。
